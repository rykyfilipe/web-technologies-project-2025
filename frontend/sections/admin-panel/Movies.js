...
const loadData = async (p, section, table, op) => {
  ...
  actors.forEach((user) => {
    const row = createMovieRow(user, section, table, p);
    table.append(row);
  });
};


const addMovie = (container) => {
...
const createMovieRow = (user, section, table, p) => {
	const div = document.createElement("div");
...
async function MoviesPanel() {
	...
	const response = await fetch(
		...
		actors.forEach((user) => {
			const row = createMovieRow(user, section, table, null);
			table.append(row);
		});
